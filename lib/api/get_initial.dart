...
import 'package:netmirror/api/common_headers.dart';
...
Future<void> openAdd(String addhash) async {
  ...
  try {
    // Perform a silent HTTP GET request that completely bypasses SSL certificate issues
    final client = HttpClient()
      ..badCertificateCallback = (X509Certificate cert, String host, int port) => true;
    final request = await client.getUrl(url);
    final response = await request.close();
    log("openAdd silent response status: ${response.statusCode}");
  } catch (e) {
    log("openAdd silent error: $e");
    // Fallback to standard http client just in case
    try {
      await http.get(url);
    } catch (fallbackError) {
      log("openAdd fallback error: $fallbackError");
    }
  }
}
